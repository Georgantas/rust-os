...
impl BootInfoFrameAllocator {
    ...
    /// Returns an iterator over the usable frame specified in the memory map.
...
/// physical memory is mapped to virtual memory at the passes `physical_memory_offset`.
...
///
/// This function is unsafe because the caller must guaruntee that the complete
...
// not required, using Translate::translate_addr from the x86_64 package instead
// /// Translates the given virtual address to the mapped physical address, or
// /// `None` if the address is not mapped.
// ///
// /// This function is unsafe because the caller must guarantee that the complete physical memory
// /// is mapped to virtual at the passed `physical_memory_offset`.
...
// /// the whole body of unsafe functions as unsafe block. This function must only
